...
/* This class is pretty much copy/pasted from LibGDX's SelectBox.java source with
 * the draw method changed. I would just extend SelectBox and override draw but
 * I needed access to private fields to implement the drop down menu, so I figured
 * I'd just make a new class with almost identical code.
 */

public class DropDownMenu<T> extends Widget implements Disableable {
	...
	public DropDownMenu(Skin skin, String name){
		this(skin.get(SelectBoxStyle.class), name);
		this.name = name;
	}
	
	public DropDownMenu(Skin skin, String styleName, String name){
		this(skin.get(styleName, SelectBoxStyle.class), name);
		this.name = name;
	}
	
	public DropDownMenu(SelectBoxStyle style, String name) {
		setStyle(style);
	...
	public void draw (Batch batch, float parentAlpha) {
		...
		if (selected != null) {
			...
			int numGlyphs = font.computeVisibleGlyphs(name, 0, name.length(), availableWidth);
			bounds.set(font.getBounds(name));
			height -= background.getBottomHeight() + background.getTopHeight();
			float textY = (int)(height / 2 + background.getBottomHeight() + bounds.height / 2);
			font.setColor(fontColor.r, fontColor.g, fontColor.b, fontColor.a * parentAlpha);
			font.draw(batch, name, x + background.getLeftWidth(), y + textY, 0, numGlyphs);
		}
	}
	...
	class ListScroll extends ScrollPane {
		...
		public void show (Stage stage) {
			...
			stage.addActor(this);
			DropDownMenu.this.localToStageCoordinates(tmpCoords.set(0, 0));
			...
			float heightAbove = stage.getCamera().viewportHeight - tmpCoords.y - DropDownMenu.this.getHeight();
			boolean below = true;
			...
			if (below)
				...
				setY(tmpCoords.y + DropDownMenu.this.getHeight());
			
			setX(tmpCoords.x);
			setWidth(DropDownMenu.this.getWidth());
			setHeight(height);
